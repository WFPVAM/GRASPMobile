    ...
    public FormEntryPrompt[] getQuestionPrompts() throws RuntimeException {
        ...
        if (mFormEntryController.getModel().getForm().getChild(currentIndex) instanceof GroupDef) {
            ...
            for (int i = 0; i < indicies.size(); i++) {
                ...
                {
                    //*****************
                    mFormEntryController.getModel().getQuestionPrompt(index).isReadOnly();
                    mFormEntryController.getModel().getQuestionPrompt(index).getDataType();
                    mFormEntryController.getModel().getQuestionPrompt(index).isRequired();
                    //*****************
