...
public class InstanceProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        if (values.containsKey(InstanceColumns.DISPLAY_SUBTEXT) == false) {
            Date today = new Date();
            String text = getDisplaySubtext(InstanceProviderAPI.STATUS_SAVED, today);
            values.put(InstanceColumns.DISPLAY_SUBTEXT, text);
        }
        
        if (values.containsKey(InstanceColumns.STATUS) == false) {
            values.put(InstanceColumns.STATUS, InstanceProviderAPI.STATUS_SAVED);
        }
    ...
    private String getDisplaySubtext(String state, Date date) {
        ...
        if (state == null) {
            return "Added on " + ts;
        } else if (InstanceProviderAPI.STATUS_SAVED.equalsIgnoreCase(state)) {
            return "Saved on " + ts;
        } else if (InstanceProviderAPI.STATUS_COMPLETED.equalsIgnoreCase(state)) {
