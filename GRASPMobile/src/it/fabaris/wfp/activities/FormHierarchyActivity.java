			...
			switch (event)
			{
																	...
																	if ((fp.getAnswerText() == null || fp.getAnswerText() == "")
																		...
																		if(!(!fp.isRequired() && fp.isReadOnly() && !fp.getFormElement().getBind().getReference().toString().toLowerCase().contains("vis") && fp.getDataType() == 3))
																		{
																			formList.add(new HierarchyElement(fp.getLongText(), fp
																				.getAnswerText(), null, colorHelper, HierarchyElement.QUESTION, fp
																				.getIndex()));
																		}
				...
				case FormEntryController.EVENT_REPEAT:
															...
															if (repeatedGroupRef.equalsIgnoreCase("") && fc.getMultiplicity() == 0)
															{
																 ...
																 *  of its children
																 */
																HierarchyElement group = new HierarchyElement(fc.getLongText(), "", getResources().getDrawable(R.drawable.expander_ic_minimized), colorHelper, HierarchyElement.COLLAPSED, fc.getIndex());
															...
															if (repeatedGroupRef.compareTo(FormEntryActivity.mFormController.getFormIndex().getReference().toString(false)) == 0)
															{
																...
																{
																	h.addChild(new HierarchyElement(mIndent + "" + " " + (fc.getMultiplicity() + 1), null, null, colorHelper, HierarchyElement.CHILD, fc.getIndex()));
																} 
																else
																{
																	h.addChild(new HierarchyElement(mIndent	+ fc.getLongText() + " " + (fc.getMultiplicity() + 1), null, null, colorHelper, HierarchyElement.CHILD, fc.getIndex()));
																}
															}
															break;
			}
