	...
	public void populateTemplate(TreeElement incoming, FormDef f) {
		if (this.isLeaf()) {
			...
			for (int i = 0; i < this.getNumChildren(); i++) {
				...
				if (child.repeatable) {
				    for (int k = 0; k < newChildren.size(); k++) {
				    	org.javarosa.core.model.instance.TreeElement template = f.getInstance().getTemplate(child.getRef());
				        org.javarosa.core.model.instance.TreeElement newChild = template.deepCopy(false);
				        newChild.setMult(k);
				        this.children.insertElementAt(newChild, i + k + 1);
				        //newChild.populateTemplate(org.javarosa.core.model.instance.TreeElement) newChildren.elementAt(k), f);
				    }
				    i += newChildren.size();
				} else {
